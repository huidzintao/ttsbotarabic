...
3. Перед генерацией показывает кнопки: пользователь одним нажатием меняет
   пол/возраст любого персонажа (мужчина/женщина/мальчик/девочка и т.д.).
"""
...
TG_TOKEN = os.environ["TG_TOKEN"]                       # токен от @BotFather
GEMINI_API_KEY = os.environ["GEMINI_API_KEY"]           # ключ из aistudio.google.com
TTS_MODEL = os.environ.get("TTS_MODEL", "gemini-2.5-flash-preview-tts")
TEXT_MODEL = os.environ.get("TEXT_MODEL", "gemini-2.5-flash")
SERVICE_URL = os.environ.get("SERVICE_URL", "").rstrip("/")  # https://xxx.run.app (заполним после деплоя)
...
# ----------------------- ГОЛОСА (подтверждено get_model_info в этом сеансе) -----------------------
# tag -> (имя голоса Gemini, инструкция как говорить)
ROLE_VOICE = {
...
# порядок переключения по кнопке
...
1) Явные префиксы важнее всего: الشيخ/المعلم/الأستاذ -> m-mid; الأستاذة/المعلمة -> f-mid;
   الأم/المعلمة/الخالة -> f; الأب/العم/الخال -> m; الطفل/الولد/الصبي -> m-teen; الطفلة/البنت -> f-teen;
...
4) Глагольные окончания говорящего: فعلتْ/ذهبتْ/كتبتْ (та марбута) -> f; فعلَ/ذهبَ/كتبَ -> m.
...
def main():
    ...
    if SERVICE_URL:
        # Режим для Cloud Run: Telegram сам шлёт обновления на наш HTTPS-адрес.
        ...
        # Локальный тест на своём компьютере.
        log.info("Polling mode (локальный тест)")
        app.run_polling()
